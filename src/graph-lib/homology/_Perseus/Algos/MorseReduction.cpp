...
num MComplex<C,BT>::freeCofaceCollapser()
{
	...
	std::vector<typename CLIST::iterator> tokill;
	typename CLIST::iterator killhere;

	typename std::vector<typename CLIST::iterator>::iterator removeus;
	BT birth;
...
template <typename C, typename BT>
void MComplex<C,BT>::enqueue(const CCHAIN& mychain, std::deque<Cell<C,BT>*>& Queue)
{
...
template <typename C, typename BT>
void MComplex<C,BT>::updateGpathNearQueen_Cored(Cell<C,BT>* queen, std::deque<Cell<C,BT>*>& Queue, bool rightdim)
{
	rightdim = true;
...
// additionally enques all coboundary cells with isQueued = false to myQ!
template <typename C, typename BT>
void MComplex<C,BT>::updateGpathNearKing_Red(Cell<C,BT>* king, std::deque<Cell<C,BT>*>& Queue, bool rightdim)
{
	rightdim = true;
...
template <typename C, typename BT>
bool MComplex<C,BT>::markPair_Red(Cell<C,BT>* queen, Cell<C,BT>* king, std::deque<Cell<C,BT>*>& Queue, num rootdim,
...
// gpaths were manipulated
template <typename C, typename BT>
bool MComplex<C,BT>::markPair_Cored(Cell<C,BT>* king, Cell<C,BT>* queen, std::deque<Cell<C,BT>*>& Queue, num rootdim,
...
void MComplex<C,BT>::removeFreeFace(Cell<C,BT>* torem, bool deep=true)
{
	...
	if (FFTALK)
	{
		...
		std::cout<<" coface has boundary: "<<coface->getBD();
	}
	...
	{
		std::cout<<"\n\n Removed!!";
		std::cout<<" now, coface has boundary: "<<coface->getBD();
		std::cin.get();
	}
...
num MComplex<C,BT>::getNextCrit_Red (const BT& birth, Cell<C,BT>*& newace, bool deep = true)
{
	...
	std::pair<num,bool> mdret = this->getMaxDim(birth);
	...
	do // while there are uncritical cells...
	{
		...
		for (lit = curl->begin() + minindex; lit <= curl->begin() + finalind; ++lit)
		{
			...
			if (newace->isFreeCoface(newace, face, true))
			{
...
num MComplex<C,BT>::getNextCrit_Cored (const BT& birth, Cell<C,BT>*& newace, bool deep = true)
{
	...
	// extract dimension with next critical cell
	std::pair<num,bool> mdret = this->getMinDim(birth);
	// check if there are no more cells left in this dimension
	if (mdret.second == false) return 0;
	...
	do // while there are uncritical cells...
	{
		...
		for (lit = curl->begin() + minindex; lit <= curl->begin()+ finalind; ++lit)
		{
			...
			// if we reach here, the cell is _not_ critical and may be checked for ffc!
			if (newace->isFreeFace(newace, coface, true))
			{
...
template <typename C, typename BT>
std::pair<num,int> MComplex<C,BT>::Reduce(const BT& birth = INITBT, const BT& ignore = BANBT,
									 bool deep = true, bool tracer = false)
{
	...
	else return std::make_pair(0,-1); // frame does not exist
	...
	if (ace == NULL) return std::make_pair(numrem,-1);
	std::vector<typename CLIST::iterator> toKill;

	// now the routine

	std::deque<Cell<C,BT>*> Queue; // create empty queue of cell pointers
	Queue.clear();
	...
	std::vector<Cell<C,BT>*> flushMe; // this list holds all uncritical cells that are
	...
	while(!Queue.empty())
	{
		...
		if (queen->getCB().size() == 1) // if queen has nontrivial coboundary,...
		{
			...
			// check for kq pair
			if (queen->isQKPair(queen, king, true))
			{
				if (AKQTALK || tracer)
				{
					debout<<"\n  King: "<<*king<<" at "<<king;
	...
	typename std::vector<typename CLIST::iterator>::iterator killme;
	for (killme = toKill.begin(); killme != toKill.end(); ++killme)
	{
		delete **killme;
		**killme = NULL;
	}
	return std::make_pair(numrem,1); // number of collapses performed
} // coreduce
...
template <typename C, typename BT>
std::pair<num,int> MComplex<C,BT>::CoReduce(const BT& birth = INITBT, const BT& ignore = BANBT,
									   bool deep = true, bool tracer = false)
{
	isred = false;
	...
	if(this->fsize(birth) != 0)
	{
		if (CORETALK || CMFTALK || tracer)
		{
			...
			debout<<" vs min "<<getMinDim(birth).first; std::cin.get();
		}
	}
	else return std::make_pair(0,-1); // frame does not exist
	...
	while(!Queue.empty())
	{
		...
		if (king->getBD().size() == 1) // if king has nontrivial boundary,...
		{
			...
			// check for kq pair
			if (queen->isKQPair(king, queen, true))
			{
				if (AKQTALK || tracer)
				{
					debout<<"\n  King: "<<*king<<" at "<<king;
...
template <typename C, typename BT>
std::pair<num,bool> MComplex<C,BT>::RedMorsify(const BT& birth, bool deep = true,
										  BT except = BANBT,bool tracer=false)
{
   //if (birth == 5) tracer = true;
	...
	std::pair<num,int> coret; // stores return value of coreduce
	std::pair<num,bool> oldmdret, newmdret; // stores result of critical dimension computations
	int numrem = 0;
	num numiter = 0;
	do
	{
		...
		case -1: // frame does not exist...
			...
			minindex = 0;
			//cin.get();
			return std::make_pair(numrem, false);
		case 1: // frame exists, we can coreduce some more
			...
			if (!hasUncritCells(birth))
			{
				...
				return std::make_pair(numrem,true);
			}
...
// statuses: (the second integer returned value):
template <typename C, typename BT>
std::pair<num,bool> MComplex<C,BT>::CoredMorsify(const BT& birth, bool deep = true,
											BT except = BANBT, bool tracer = false)
{
	minindex = 0;
	typename COMPLEX::iterator cit;
...
num MComplex<C,BT>::MorseCoreduceExcept(const BT& start = INITBT, const BT& exception = BANBT, bool savegens = false)
{
	...
	for(cit = this->clist.lower_bound(start); cit != this->clist.end(); ++cit)
	{
		...
		// now we morse-reduce
		numrem = CoredMorsify(birth, !savegens, exception, false); // deep removal if generators are not needed!
	...
	for (fit = touncrit->rbegin(); fit != touncrit->rend(); ++fit)
	{
		...
		for (lit = curlist->begin(); lit != curlist->end(); ++lit)
		{
			...
			if (curcell->gpath != NULL)
			{
				...
				//if (MAKEBPS) cin.get();
				curcell->setCB(*(curcell->gpath), true);
	...
	for (fit = touncrit->begin(); fit != touncrit->end(); ++fit)
	{
		...
		for (lit = curlist->begin(); lit != curlist->end(); ++lit)
		{
			...
			if (curcell->gpath != NULL)
			{
				...
				//if (MAKEBPS) cin.get();
				curcell->setBD(*(curcell->gpath), true);
				delete curcell->gpath;
				curcell->gpath = NULL;
			}
		}
...
num MComplex<C,BT>::MorseWrapper_Cored(bool savegens = false, double thresh = 0.3)
{
	...
	do
	{
		//cout<<"in..."; cin.get();
		oldsz = this->size();
		premct = MorseCoreduce(savegens, false);
		//cout<<*this;
		totrem += premct;
...
/// **********************************************************************
template <typename C, typename BT>
void MComplex<C,BT>::CoreduceAndUpdate (MComplex<C,BT>*& mycomp, bool savegens = false,
						double thresh = 0.5, bool superopt = false, std::ostream& out = std::cout)
{
	...
	do
	{
		//cout<<"in..."; cin.get();
		oldsz = mycomp->size();
		premct = mycomp->MorseCoreduce(savegens, false);
		fracrem = (oldsz == 0) ? 1 : float(2*premct)/float(oldsz);
		//cout<<ccomp;
		//cin.get();
		if (FLOWTALK)
...
template <typename C, typename BT>
void MComplex<C,BT>::AlternateAndUpdate_makeStat (MComplex<C,BT>*& mycomp,  std::vector<num>& svec,
								  double thresh = 0.0)
{
	svec.clear();
	double movewhen = 0.0;
...
template <typename C, typename BT>
num MComplex<C,BT>::AlternateAndUpdate (MComplex<C,BT>*& mycomp,  bool savegens = false, double thresh = 0.5,
						 bool redfirst = false, bool superopt = false, std::ostream& out = std::cout)
{
...
template <typename C, typename BT>
void MComplex<C,BT>::ReduceAndUpdate (MComplex<C,BT>*& mycomp, bool savegens = false,
					  double thresh = 0.5, bool superopt = false, std::ostream& out = std::cout)
{
	MComplex<C,BT>* rcomp = NULL; // reduced complex!
	num premct, oldsz; // number of pairs removed in each iteration of the morse tower
	...
	do
	{
		//cout<<"in..."; cin.get();
		oldsz = mycomp->size();
		premct = mycomp->MorseReduce(savegens, false);
		fracrem = (oldsz == 0) ? 1 : float(2*premct)/float(oldsz);
		//cout<<ccomp;
		//cin.get();
		if (FLOWTALK)
...
template <typename C, typename BT>
void MComplex<C,BT>::AlternateAndUpdate_Shuffle (MComplex<C,BT>*& mycomp,  num attempts=0, std::ostream& out = std::cout)
{
	MComplex<C,BT>* rcomp = NULL; // reduced complex!
	num oldsz, newsz;
...
template <typename C, typename BT>
void MComplex<C,BT>::flushQ(const std::vector<Cell<C,BT>*>& toflush)
{
	//cout<<"\n flushQ size: "<<toflush.size();
   //cin.get();
	typename std::vector<Cell<C,BT>*>::const_iterator lit;
