...
template <typename C, typename BT>
void Cell<C,BT>::showGens(std::ostream& out = std::cout, bool samebirth = false, const std::string buf = "------ ") const
{
}

// prints cell as a triple of dimension, index and birth time
template <typename C, typename BT>
void Cell<C,BT>::print(std::ostream& out) const
{
...
template <typename C, typename BT>
bool Cell<C,BT>::isFreeFace(const Cell<C,BT>* face, Cell<C,BT>*& coface, bool checkbt=true)
{
...
// birth time equality in case of persistence
template <typename C, typename BT>
bool Cell<C,BT>::isFreeCoface(const Cell<C,BT>* coface, Cell<C,BT>*& face, bool checkbt=true)
{
...
template <typename C, typename BT>
bool Cell<C,BT>::isKQPair(const Cell<C,BT>* k, const Cell<C,BT>* q, bool checkbt=true)
{
...
template <typename C, typename BT>
bool Cell<C,BT>::isQKPair(const Cell<C,BT>* q, const Cell<C,BT>* k, bool checkbt=true)
{
	// nothing serious here now, but eventually:
	// check if the incidence coefficient is a unit!
...
bool Chain<C,D>::addLink (const D& ptr, const C& coeff) // add a single cell with coefficient given
{
	...
	if (coeff != 0)
	{
		...
		else cells.insert(std::make_pair( (D)ptr, (C) coeff ) );
	}
...
{
	std::vector<typename CHAIN::iterator> tokill; // store everything to kill
	...
	typename std::vector<typename CHAIN::iterator>::iterator kit;
...
num Chain<C,D>::removeUnmarked(bool trace=false)
{
	...
	std::vector<typename CHAIN::iterator> unmit;
	...
	typename std::vector<typename CHAIN::iterator>::iterator vit;
