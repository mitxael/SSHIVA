...
public:
	...
	std::vector<num>* addin;	// dimensions to increment by ES
	...
	Cube(std::vector<num>* add, Point<PS>* anch)
	...
	void print (std::ostream& out) const // print helper
	{
		typename std::vector<num>::iterator add_iter;
		if (anchor != NULL)
		{
			out<<"[ "<<*anchor<<" ; { ";
			if (addin != NULL)
	...
	void setAddin(const std::vector<num>*); // set addin vector
