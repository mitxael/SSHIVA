...
{
public:
	std::vector<PS> coords; // vector of coordinates
	num hash; // hash value
		// silly constructor
	Point()
	{
	...
	friend std::ostream& operator << (std::ostream& out, const Point<PS>& toprint)
	{
		typename std::vector<PS>::const_iterator coord;
		out<<"( ";
	...
	void setCoords(const std::vector<PS>&); // sets coordinates to given vector
	...
	typename std::vector<PS>::const_iterator begin() const; // begin coordinate iter
	typename std::vector<PS>::const_iterator end() const;  // end coordinate iter
