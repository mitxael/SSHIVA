...
void Point<PS>::setCoords(const std::vector<PS>& co)
{
	coords = co;
}
...
// iterate over coords!
template <typename PS>
typename std::vector<PS>::const_iterator Point<PS>::begin() const
{
	return coords.begin();
}

template <typename PS>
typename std::vector<PS>::const_iterator Point<PS>::end() const
{
	return coords.end();
}
