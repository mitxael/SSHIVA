...
void make3Dvector(num dim1_size, num dim2_size, std::vector<std::vector<std::vector<datatype>*>*>& ans)
{
	for (num i = 0; i < dim1_size; ++i)
	{
		ans.push_back(new std::vector<std::vector<datatype>*>);
		for(num j=0; j < dim2_size; ++j)
		{
			ans.at(i)->push_back(new std::vector<datatype>);
		}
	}
...
template <typename datatype>
num getVPos(const std::vector<datatype>& svec, const datatype toget)
{
	typename std::vector<datatype>::const_iterator locate = lower_bound(svec.begin(),svec.end(),toget);
...
bool lexicoPos (const num n, const num k, const num p, std::vector<num>& ans)
{
...
template <typename T>
T vProd (const std::vector<T> tomult)
{
	T ans = 1;
...
num lexicoPosRev(num n, const std::vector<num> subset)
{
...
template<class T, class S> void decompose(T n, const std::vector<T>& extents, std::vector<S>& ans)
{
	ans.clear();
...
template <typename T>
void makeProd (const std::vector<T>& exts, std::vector<T>& ans)
{
	num d = exts.size();
...
template <typename  T>
T recompose(const std::vector<T>& coords, const std::vector<T>& exts)
{
	// get product:
	std::vector<T> prods;
	makeProd(exts,prods);
