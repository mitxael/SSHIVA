...
void CToplex<C,PS,BT>::insertFacePairOf(Cell<C,BT>* cur, std::vector<num>* add, Point<PS>* anc1,
										Point<PS>* anc2, const C coeff)
{
	...
	{
		std::cout<<"\n    Search addin: < ";
		for(unsigned int i=0; i<add->size(); i++) std::cout<<add->at(i)<<" ";
		std::cout<<">";
	}

	if (addfind == mymap.end()) // no such addin exists
	{
		if (CUBFACETALK) std::cout<<"... not found!";
		...
		cur->addBDLink(face1,coeff, true, true);
		...
		cur->addBDLink(face2,-coeff, true, true);
		(*rgmap)[anc2] = face2;

		if (CUBFACETALK) std::cout<<"\n          Inserting: "<<*face1<<" and "<<*face2;
		// okay, insert this range map into mymap corresponding to addin
		mymap.insert(mymap.begin(), pair<std::vector<num>*,CUBE_RGMAP*>(add,rgmap));;
	}
...
{
	std::vector<num>* curaddin = additer->first;
	Point<PS>* curanc = rgiter->first;

	Cell<C,BT>* curcube = rgiter->second;

	std::vector<num>* faceaddin; // addin of face
	...
	for (num cdim = 0; cdim < 2*curcube->getDim()-1; cdim+=2)
	{
		...
		// CURRENT FACE
		faceaddin = new std::vector<num>(*(curaddin));
...
void CToplex<C,PS,BT>::addCubeFaces()
{
	...
	for (additer = mymap.rbegin(); additer != mymap.rend(); ++additer)
	{
		...
		{
			std::cout<<"\n  Handling Addin: < ";
			for (unsigned int i=0; i<curaddin->size();i++) std::cout<<curaddin->at(i)<<" ";
			std::cout<<">";
			std::cin.get();
		}
...
template <typename C, typename PS, typename BT>
bool CToplex<C,PS,BT>::buildTopCubeMap(const num topdim, const std::vector<Point<PS>*>& anchs, const std::vector<BT>* births = NULL)
{
	...
	typename std::vector<Point<PS>*>::const_iterator curpt;
	...
	if (mymap.size()==0)
	{
		...
		std::vector<num>* topadd = new std::vector<num>;
	...
	for (curpt = anchs.begin(); curpt != anchs.end(); ++curpt)
	{
		...
		if (!(rgmap->insert(std::make_pair(*curpt,curcube)).second))
		{
...
template <typename C, typename PS, typename BT>
bool CToplex<C,PS,BT>::makeCellComplex(const num topd, const std::vector<Point<PS>*>& ancs, MComplex<C,BT>& mycomp)
{
...
template <typename C, typename PS, typename BT>
std::pair<num,bool> CToplex<C,PS,BT>::readFileInfo(std::ifstream& infile, num& topdim, PB_MAP& anchors)
{
	num count = 0;
	...
	BT birthtime;
	std::pair<typename PB_MAP::iterator, bool> insres;

	// POINT STUFF
	PS curcoord; // current coordinate being read from file
	std::vector<PS> ptcoords; // coordinates of anchor point
	...
	while(infile.good())
	{
		...
		insres = anchors.insert(std::make_pair(anchor,birthtime));
		if (!insres.second)
		{
...
template <typename C, typename PS, typename BT>
std::pair<num,bool> CToplex<C,PS,BT>::makeFromFile(std::ifstream& infile)
{
	num topdim;
	PB_MAP anchors;
	// read point-birth info from the file!
	std::pair<num,bool> toret = readFileInfo(infile, topdim, anchors);
	//cout<<" read "; cin.get();
	if (toret.second == false) return toret;
	//cout<<"count: "<<count; cin.get();
	std::vector<Point<PS>*> ancvec;
	std::vector<BT>* births = new std::vector<BT>;
...
void CToplex<C,PS,BT>::removeAllBut(const BT& except)
{
	...
	std::vector<typename CUBE_RGMAP::iterator> tokill;
	// and an iterator over these
	typename std::vector<typename CUBE_RGMAP::iterator>::iterator killme;
...
template <typename C, typename PS, typename BT>
MComplex<C,BT>* CToplex<C,PS,BT>::readMovie(std::ifstream& infile)
{
	num topdim;
	PB_MAP anchors;
	...
	{
		bmap.insert(std::make_pair(pbit->second,pbit->first));
	...
	// iterate over births
	typename std::set<BT>::const_iterator bit;
	...
	std::vector<Point<PS>*> ancvec;
	std::vector<BT>* bvec = new std::vector<BT>;
	...
	for (cur_it = 0; cur_it <= iterations; ++cur_it)
	{
		...
		writeComplexExcept(*(curcomp),lowval-1, false);
