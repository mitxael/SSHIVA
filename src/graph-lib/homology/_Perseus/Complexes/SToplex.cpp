...
bool SToplex<C,PS,BT>::addCoface(Simplex<C,PS,BT>* toin)
{
	...
	rg->insert(std::make_pair(toin->getHash(getHashMod()),toin));
...
Simplex<C,PS,BT>* SToplex<C,PS,BT>::isIn (const PT_SET& tofind)
{
	...
	std::pair<typename SIMP_RG::iterator, typename SIMP_RG::iterator> ancrange;
...
bool SToplex<C,PS,BT>::makeFacesOf(Simplex<C,PS,BT>* simp, bool inheritBirths = true)
{
	...
	for (hat = simp->begin(); hat != simp->end(); ++hat)
	{
		...
		if (sloc == NULL) // no such face exists
		{
			...
			currg->insert(std::make_pair(sloc->getHash(getHashMod()),sloc));
...
template <typename C, typename PS, typename BT>
bool SToplex<C,PS,BT>::buildTopSimpMap (const std::vector<Simplex<C,PS,BT>*>& topsimps, bool makeverlist = true)
{
	verlist.clear();
	...
	std::set<Point<PS>*, ptcomplex<PS> > vertices; // store all vertices
	typename PT_SET::const_iterator viter; //

	// iterate over top simplices and extract vertices:
	typename std::vector<Simplex<C,PS,BT>*>::const_iterator curtop;
...
template <typename C, typename PS, typename BT>
void SToplex<C,PS,BT>::writeComplex(MComplex<C,BT>& simpcomp) 
{
...
template <typename C, typename PS, typename BT>
void SToplex<C,PS,BT>::printSizeInfo (std::ostream& out = std::cout)
	{
...
template <typename C, typename PS,typename BT>
bool SToplex<C,PS,BT>::makeCellComplex(const std::vector<Simplex<C,PS,BT>*>& topsimps, MComplex<C,BT>& simpcomp, bool inheritBirths = true)
{
...
template <typename C, typename PS,typename BT>
num SToplex<C,PS,BT>::populateMap(const std::vector<Simplex<C,PS,BT>*>& topsimps, bool inheritBirths = true)
{
...
template <typename C, typename PS, typename BT>
std::pair<num,bool> SToplex<C,PS,BT>::makeFromFile(std::ifstream& infile, bool hasbirths = true,
											  bool mfd = true, bool hasdimcap = false)
{
	...
	if(infile.good()) infile >> ptdim; else return std::make_pair(0,false);
	...
	std::map<Point<PS>*, num, ptcomplex<PS> > pts; // set of all points and corresponding indices
	typename std::map<Point<PS>*, num, ptcomplex<PS> >::iterator finder;

	// map from set of vertices to birth times...
	std::map<PT_SET, BT> vertbirth;
	std::pair<typename std::map<PT_SET, BT>::iterator, bool> insres; // result of inserting into map
	typename std::map<PT_SET, BT>::iterator curptsetpos; // position of current point-set in vertbirth


	if((topdim < 0) || ptdim==0) return std::make_pair(count,false); // negative dimension count??? i quit!
	...
	while (infile.good())
	{
		...
		for (num j = 0; j <= topdim; j++)
		{
			...
			if (finder == pts.end())
			{
				...
				pts.insert(pts.end(),std::make_pair(curpt,curptind++));
				//cout<<"\n adding pt: "<<*curpt;
			}
		...
		insres = vertbirth.insert(std::make_pair(curptset,curborn));
	...
	std::vector<Simplex<C,PS,BT>*> topsimps;
	// loop over vertbirths
	//cout<<"szinfo0"; cin.get();

	typename std::map<PT_SET,BT>::const_iterator vset;
	...
	addSimpFaces(true); // and add faces recursively.
	if (SIMTOPTALK)
	{
		std::cout<<"\n faces made ";
		if (MAKEBPS) std::cin.get();
...
template <typename C, typename PS, typename BT>
num SToplex<C,PS,BT>::capDimension(std::map<PT_SET,BT>& vertbirth, num dimcap)
{
	num n, k = dimcap+1; // for n choose k
	bool trace = false;

	// to iterate over the top simplices!
	typename std::map<PT_SET,BT>::iterator vset;
	typename std::map<PT_SET,BT>::iterator finder; // result of insert attempt!
	std::vector<Point<PS>*> ptvec;
	PT_SET facepts;
	std::vector<num> faceverts;
	std::map<PT_SET,BT> toadd; // pairs to add to vertbirth
	std::vector<typename std::map<PT_SET,BT>::iterator > torem; // pairs to remove from vertbirth
	...
	for (vset = vertbirth.begin(); vset != vertbirth.end(); ++vset)
	{
		...
		for (num p = 1; p <= nChoosek(n,k); ++p)
		{
			...
			if (finder == toadd.end()) // this face is not already present...
			{
				...
				toadd.insert(std::make_pair(facepts,vset->second));
			}
	...
	// iterators in torem
	typename std::vector<typename std::map<PT_SET,BT>::iterator >::iterator killme;
	...
	typename std::map<PT_SET,BT>::iterator addme;
	std::pair <typename std::map<PT_SET,BT>::iterator, bool> insres; // result of insertion attempt
...
template <typename C, typename PS, typename BT>
bool SToplex<C,PS,BT>::topWrapper(const std::vector<Simplex<C,PS,BT>*>& topsimps, std::string fname = "output")
{
...
bool SToplex<C,PS,BT>::barySub(SToplex<C,PS,BT>& sd)
{
	...
	std::map<Simplex<C,PS,BT>*,Simplex<C,PS,BT>*> bcent;
	...
	for (mit = mymap.begin(); mit != mymap.end(); ++mit)
	{
		...
		for (rit = currg->begin(); rit != currg->end(); ++rit)
		{
			...
			if (BARYTALK) std::cout<<"\n -- Map "<<*sigma<<" to "<<simpcount;

			bcent.insert(std::make_pair(sigma,bcsigma));
			simpcount++;
		}
	}
...
template <typename C, typename PS, typename BT>
bool SToplex<C,PS,BT>::subdivide (Simplex<C,PS,BT>* sigma, std::map<Simplex<C,PS,BT>*,
								  Simplex<C,PS,BT>*>& bc, std::set<PT_SET>& result)
{
	...
	std::set<PT_SET> taures; // stores result of subdivide(tau)
	typename std::set<PT_SET>::iterator tit;
	PT_SET curset;
...
bool SToplex<C,PS,BT>::insertVertex (Simplex<C,PS,BT>*& tau)
{
	...
	currg->insert(std::make_pair(tau->getHash(this->getHashMod()),tau));
	return true;
}
...
// creates simplicial toplex from file input
template <typename C, typename PS, typename BT>
std::pair<num,bool> SToplex<C,PS,BT>::makeRicci(std::ifstream& tetfile, std::ifstream& edgefile)
{
	num count = 0; // total number of top simplices read
	num topdim=0; // top dimension of simplices encountered
	...
	while (tetfile.good())
	{
		...
		insres = vertbirth.insert(std::make_pair(curptset,INITBT));
		curptsetpos = insres.first; // extract location in vertbirth of curptset
...
template <typename C, typename PS, typename BT>
bool SToplex<C,PS,BT>::readEdgeBirths(std::ifstream& edgefile,
						std::map<Point<PS>*, num, ptcomplex<PS> >& pts)
{
