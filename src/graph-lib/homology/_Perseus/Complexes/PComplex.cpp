...
void PComplex<C,BT>::initPersData(const Complex<C,BT>& other)
{
	...
	for(i=other.clist.begin(); i != other.clist.end() ; ++i)
	{
		...
		betti[i->first] = new std::vector<BT>;
...
template <typename C, typename BT>
void PComplex<C,BT>::showInts(std::ostream& out = std::cout)
{
	//out<<"\n\n:::::INTERVALS::::::";
...
template <typename C, typename BT>
void PComplex<C,BT>::showBetti(std::ostream& out = std::cout)
{
	//out<<"\n\n:::::BETTI NUMBERS BY FRAME:::::\n\n"; //cin.get();
	typename BETTI_STR::const_iterator bit;
	typename std::vector<BT>::const_iterator vit;
	std::vector<BT>* curvec;
...
template <typename C, typename BT>
std::map<num,std::vector<std::pair<BT,BT> > > PComplex<C,BT>::getInts() const
{
	// return interval structure
	std::map<num,std::vector<std::pair<BT,BT> > > toret;
	typename INT_STR::const_iterator it;
	INTVEC* curints;
	typename INTVEC::const_iterator iit;

	std::vector<std::pair<BT, BT> > newints; // intervals for current dimension...
	for (it = ints.begin(); it != ints.end(); ++it)
	{
		newints.clear();
		curints = it->second;
		toret.insert(std::make_pair(it->first,newints));
		for (iit = curints->begin(); iit != curints->end(); ++iit)
		{
...
template <typename C, typename BT>
bool PComplex<C,BT>::makeOutputFiles(const std::string& filename)
{
	...
	INTVEC* curints;
	typename INTVEC::const_iterator iit;
	std::string fname; // file name for writing
	for(it = ints.begin(); it != ints.end(); ++it)
	{
	...
	std::ofstream bfile(fname.c_str());
