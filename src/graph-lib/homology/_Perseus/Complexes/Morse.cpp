...
void MComplex<C,BT>::moveOver_Sort(MComplex<C,BT>& other, bool forred)
{
	...
	std::multiset<Cell<C,BT>*,unitbdord<C,BT> > bdsort;
	std::multiset<Cell<C,BT>*,unitcbord<C,BT> > cbsort;

	// and their iterators:
	typename std::multiset<Cell<C,BT>*, unitbdord<C,BT> >::iterator bditer; // for reduce
	typename std::multiset<Cell<C,BT>*, unitcbord<C,BT> >::iterator cbiter; // for coreduce
...
template <typename C, typename BT>
double MComplex<C,BT>::boundaryMatrix (const num& updim, std::vector<std::vector<C> >& bmat,
									 BT birth = BANBT) const
{
	...
	bmat.assign(upsz, std::vector<C>(dnsz,INITBT));
...
void MComplex<C,BT>::moveOver_Shuffle(MComplex<C,BT>& other)
{
	...
	std::srand(std::time(NULL)); // for random shuffle!
...
template <typename C, typename BT>
void MComplex<C,BT>::showGens (const BT& born, const num& dim, bool resbirth = false, std::ostream& out = std::cout) const
{
...
template <typename C, typename BT>
void MComplex<C,BT>::quicksert(const std::vector<Cell<C,BT>*>& toins)
{
	if (toins.size() > 0)
	{
...
template <typename C, typename BT>
std::pair<num,bool> MComplex<C,BT>::getMaxDim(const BT& btime)
{
	...
	if (this->sizeinfo.find(btime) == this->sizeinfo.end()) return std::make_pair(0,false);
	...
	for (dimit = cursz->rbegin(); dimit != cursz->rend(); ++dimit)
	{
		...
		{
			return std::make_pair(dimit->first, true); // return this dimension + true!
		}
	}
...
template <typename C, typename BT>
std::pair<num,bool> MComplex<C,BT>::getMinDim(const BT& btime)
{
...
void MComplex<C,BT>::memorySaver(BT birth = INITBT)
{
	...
	std::vector<std::pair<BT, num> > prunelists;
	...
	for (compit = this->clist.begin(); compit != this->clist.end(); ++compit)
	{
		...
		for (frit = curf->begin(); frit != curf->end(); ++frit)
		{
			...
			if (2*this->lsize(curbirth,curdim) <= (num)curl->size() )
			{
				...
				prunelists.push_back(std::make_pair(curbirth,curdim));
			}
		}
	}

	// now loop over prunelists and prune them!
	typename std::vector<std::pair<BT,num> >::iterator pliter;
...
bool MComplex<C,BT>::setMinIndex(const BT birth)
{
	...
	std::pair<num,bool> mdret = isred ? getMaxDim(birth) : getMinDim(birth);
	...
	{
		std::cout<<"no mindim for birth "<<birth<<", try reset!";
		std::cin.get();
		minindex = 0;
		return false;
	}
	...
	std::pair<num,num> sz = getSizeInfo(birth,newdim);
