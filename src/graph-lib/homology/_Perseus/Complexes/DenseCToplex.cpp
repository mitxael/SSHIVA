...
bool DenseCToplex<C,BT>::makeFaces(bool incbirth = false)
{
	...
	std::vector<num> curaddin, curanch, curext;
	...
	for (rit = cube.rbegin(); rit != cube.rend(); ++rit)
	{
		...
		getAddin(i,curaddin);
		if (DCFACETALK) {std::cout<<"\n***** Facing: "; c_print<std::vector<num> >(curaddin);}
...
template <typename C, typename BT>
num DenseCToplex<C,BT>::getAddinPosFast(const std::vector<num>& advec) const
{
...
template <typename C, typename BT>
void DenseCToplex<C,BT>::makeFaceLinks(Cell<C,BT>* curcb, const std::vector<num>& addin, const std::vector<num>& anch)
{
	std::vector<num> fcaddin, fcanch; // face addin and anchor
	...
	if (DCFACETALK)
	{
		...
		c_print<std::vector<num> >(addin);
		std::cout<<" and anchor ";
		c_print<std::vector<num> >(anch);
		std::cin.get();
	}
	...
	for (num cdim = 0; cdim < 2*cubeDim; cdim+=2)
	{
		...
		if (DCFACETALK)
		{
			...
			c_print<std::vector<num> >(fcaddin);
			std::cout<<" and anchor ";
			c_print<std::vector<num> >(fcanch);
			std::cout<<" at pos ["<<fcaddpos<<","<<fcancpos<<"]";
			std::cin.get();
		}
...
template <typename C, typename BT>
void DenseCToplex<C,BT>::Init(const std::vector<num>& ext)
{
	num topdim = ext.size();
	...
	if (DCTTALK)
	{
		std::cout<<"\n\n *** Init DCT with ext vec "; c_print<std::vector<num> >(ext);
		std::cout<<"\n *** Extent: "; c_print<std::vector<num> >(extent); std::cout<<'\n';
		std::cout<<"\n *** adnum: " << addnum;
	}

	std::vector<num> curaddin, curext;
	CVEC* curvec;
	...
	for (num i = 0; i < (num)addnum; ++i)
	{
		...
		if (DCTTALK)
		{
			std::cout<<"\n#"<<i<<", addin: "; c_print<std::vector<num> >(curaddin);
			std::cout<<" and extent: "; c_print<std::vector<num> >(curext);
			std::cout<<"\n&&&& alloc vec of size "<<vProd<num>(curext)<<" for addin "; c_print<std::vector<num> >(curaddin); std::cin.get();
		}
...
template <typename C, typename BT>
bool DenseCToplex<C,BT>::getAddin(num pos, std::vector<num>& add) const // recover addin vector from lexico position
{
	pos++;
...
// stores in "ans".
template <typename C, typename BT>
bool DenseCToplex<C,BT>::getAnchor(num pos,  const std::vector<num>& advec, std::vector<num>& ans) const // recover anchor point from lexico position
{
	std::vector<num> dext;
...
// stores in "ans".
template <typename C, typename BT>
bool DenseCToplex<C,BT>::getAnchorFast(num pos, const std::vector<num>& ext, std::vector<num>& ans) const // recover anchor point from lexico position
{
	// now we have the extents, and can decompose to recover the anchor point!
...
// stores in "ans".
template <typename C, typename BT>
bool DenseCToplex<C,BT>::getAnchor(num pos,  const num addind, std::vector<num>& ans) const // recover anchor point from lexico position
{
	// extract addin vector from index!
	std::vector<num> advec;
...
template <typename C, typename BT>
void DenseCToplex<C,BT>::getExtForAddin(const std::vector<num>& advec, std::vector<num>& ext) const
{
...
template <typename C, typename BT>
num DenseCToplex<C,BT>::getAnchPos(const std::vector<num>& anch, const num addpos) const
{
...
template <typename C, typename BT>
num DenseCToplex<C,BT>::getAnchPos(const std::vector<num>& anch, const std::vector<num>& advec) const
{
	num toret = 0;
...
template <typename C, typename BT>
num DenseCToplex<C,BT>::getAnchPosFast(const std::vector<num>& anch, const std::vector<num>& ext) const
{
	return recompose<num>(anch,ext);
}

template <typename C, typename BT>
std::pair<num,bool> DenseCToplex<C,BT>::makeFromFile (std::ifstream& infile)
{
	if (CUBTOPTALK)
	{
		std::cout<<"\n file reading "; std::cin.get();
	}
	num topdim=0;
	// get top dimension of cubical set from the first line of input file
	if(!infile.eof()) infile >> topdim; else return std::make_pair(0,false);
	if(topdim<0) return std::make_pair(0,false); // negative dimension count??? i quit!
	...
	//cout<<*this;
	//cin.get();
	return std::make_pair(actct,true);
}
...
bool DenseCToplex<C,BT>::quickWriteComplex(MComplex<C,BT>& mycomp, bool savemem = true)
{
	...
	if (DCTSPEEDTALK){std::cout<<"\n\n *** quicksert with topdim = "<<topdim; std::cin.get();}
	...
	if (DCTSPEEDTALK){std::cout<<"\n  inserting births "<<topdim; std::cin.get();}
	...
	std::vector<std::vector<CVEC*>*> insertArray;
	make3Dvector<Cell<C,BT>*>(bsize,topdim,insertArray); // allocate array space...
	if (DCTSPEEDTALK)
	{
		std::cout<<"\n 3d vec called with "<<bsize<<" and "<<topdim; std::cin.get();
	}
...
//			}
		...
		if (DCTSPEEDTALK) {std::cout<<" end of line, clearing "; std::cin.get();}
		if (savemem)
		{
			curvec->clear();
	...
	if( DCTSPEEDTALK)
	{
		std::cout<<"\n Insert to cell complex.... "; std::cin.get();
	}
	...
	for (num i = 0; i < (num) insertArray.size(); ++i)
	{
		...
		cur2dvec->clear();
		std::vector<CVEC*>(*cur2dvec).swap(*cur2dvec);
		delete cur2dvec;
	}
	insertArray.clear();
	vector<std::vector<CVEC*>*>(insertArray).swap(insertArray);
	return true; // done
}

// adds a single top cube via its coordinates and birth time. Call AFTER init()
template<typename C, typename BT>
void DenseCToplex<C,BT>::addTopCube(const std::vector<num>& coords, const BT& btime)
{
	BT birth = btime;
	// store top cube information into the appropriate spot of cube structure!
...
template<typename C, typename BT>
void DenseCToplex<C,BT>::ComputePersistence(std::string fname = "output")
{
