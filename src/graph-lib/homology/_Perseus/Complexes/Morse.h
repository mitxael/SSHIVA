...
public:
	...
	void quicksert(const std::vector<Cell<C,BT>*>&); // inserts a vector of cells into the complex
	bool insertCell(Cell<C,BT>*); // inserts a single cell into complex

	std::pair<num,bool> getMinDim(const BT&); // re-evaluates minimum uncritical dimension for a given birth time
	std::pair<num,bool> getMaxDim(const BT&); // re-evaluates maximum uncritical dimension for a given birth time

	// cell removal routines
	bool removeCell(Cell<C,BT>*,typename CLIST::iterator&,bool); // removes a single cell from complex

	// generator output!
	void showGens (const BT&, const num&, bool, std::ostream&) const; // output generators

	void moveOver_Sort (MComplex<C,BT>&,bool);
	void moveOver (MComplex<C,BT>&);
	void moveOver_Shuffle (MComplex<C,BT>&);

	double boundaryMatrix (const num&, std::vector<std::vector<C> >&, BT) const;
	...
	std::pair<num,int> CoReduce(const BT&, const BT&, bool,bool);	// coreduces given frame iteratively with
														// lowest dim cell as seed
	std::pair<num,bool> CoredMorsify(const BT&,bool,BT,bool); // reduces complex via iterated coreduction, removes uncritical cells from
	...
	bool markPair_Cored(Cell<C,BT>*, Cell<C,BT>*, std::deque<Cell<C,BT>*>&, num,
	...
	void updateGpathNearQueen_Cored(Cell<C,BT>*, std::deque<Cell<C,BT>*>&, bool);
	...
	std::pair<num,int> Reduce(const BT&, const BT&, bool,bool);	// reduces given frame iteratively with highest
														// dim cell as seed
	std::pair<num,bool> RedMorsify(const BT&,bool,BT,bool); // reduces complex via iterated coreduction, removes uncritical cells from
													// memory if deep is on.
	...
	bool markPair_Red(Cell<C,BT>*, Cell<C,BT>*, std::deque<Cell<C,BT>*>&, num,
	...
	void updateGpathNearKing_Red(Cell<C,BT>*, std::deque<Cell<C,BT>*>&, bool);
	...
	void enqueue (const CCHAIN&, std::deque<Cell<C,BT>*>&);
	void memorySaver(BT);
	...
	/// MORSE REDUCTION
	void CoreduceAndUpdate (MComplex<C,BT>*& mycomp, bool savegens, double thresh, bool superopt, std::ostream& out);
	void AlternateAndUpdate_makeStat (MComplex<C,BT>*& mycomp,  std::vector<num>& svec, double thresh);
	num AlternateAndUpdate (MComplex<C,BT>*& mycomp,  bool savegens, double thresh, bool redfirst, bool superopt, std::ostream&);
	void ReduceAndUpdate (MComplex<C,BT>*& mycomp, bool savegens, double thresh, bool superopt, std::ostream& out);
	void AlternateAndUpdate_Shuffle (MComplex<C,BT>*& mycomp,  num attempts, std::ostream& out);
	void flushQ(const std::vector<Cell<C,BT>*>& toflush);
};
