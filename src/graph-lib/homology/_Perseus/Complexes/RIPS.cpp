...
bool RIPS<C,PS,BT>::addEdge(num pt1, num pt2, const BT& btime)
{
	...
	isnbr.at(pt1-1)->push_back(std::make_pair(pt2,born));
...
template <typename C, typename PS, typename BT>
void RIPS<C,PS,BT>::ComputePersistence(num upto, std::map<num, std::vector<std::pair<BT,BT> > >& pints)
{
...
// using ptdim.
template <typename C, typename PS, typename BT>
void RIPS<C,PS,BT>::ComputePersistence(num upto, std::string fname = "output", bool printcomp=false)
{
	//cout<<*this;
	...
	if(printcomp)
	{
		std::ofstream cfile("complex.txt");
		cfile << *ccomp;
		cfile.close();
	}
...
// using ptdim.
template <typename C, typename PS, typename BT>
void RIPS<C,PS,BT>::ComputePersistence_GrowBalls(num upto, std::string fname = "output")
{
	//cout<<*this;
...
// of the file
template <typename C, typename PS, typename BT>
std::pair<num,bool> RIPS<C,PS,BT>::makeFromFile_GrowBalls(ifstream& infile,
							bool comred = false, bool capped=false, bool witness=false)
{
	...
	while (infile.good())
	{
		...
		if (! (prmap.insert(std::make_pair(curpt, radius))).second )
		{
...
template <typename C, typename PS, typename BT>
std::pair<num,bool> RIPS<C,PS,BT>::makeFromDistMatrixFile(ifstream& infile, bool iscormat=true, bool isuptr = false)
{
	num numpts;
	if (!infile.eof())
	{
	...
	for (num i = 0; i < numpts; i++)
	{
		...
		for (num j = 0; j < jend; j++)
		{
			...
			if (curdist < epsilon)
			{
				...
				//cin.get();
				addEdge(i,j,INITBT);
				//isnbr.at(i)->push_back(std::make_pair(j,INITBT));
			}
			else // if dist is larger than epsilon,
			{
				...
				if (cursteps <= numsteps)
				{
					...
					//isnbr.at(i)->push_back(std::make_pair(j,cursteps + INITBT));
...
*/
			...
			if (cursteps <= numsteps)
			{
				...
				drow->push_back(std::make_pair(j,cursteps + INITBT));
			}
/*			else
			{
...
template <typename C, typename PS, typename BT>
void RIPS<C,PS,BT>::showNbrMatrix(std::ostream& out = cout) const
{
	num ptind;
	BT bir;
	...
	for (num i = 1; i < (num)vert.size(); i++)
	{
		...
		for(num j=0; j < i; j++)
		{
			...
			if (dist(*(vert.at(i)),*(vert.at(j))) < (rads.at(i)+rads.at(j)))
			{
				...
				drow->push_back(std::make_pair(j,INITBT));
			}
			else
			{
				if (RIPSNBRTALK) {cout<<"... NO!\n";}
...
template <typename C, typename PS, typename BT>
std::pair<num,bool> RIPS<C,PS,BT>::makeFromFile(ifstream& infile)
{
	// get point dimension
	num ptdim = 0;
	...
	while(infile.good())
	{
		...
		// try to insert...
		if (! (ptbirth.insert(std::make_pair(curpt,make_pair(radius,curborn)))).second )
		{
								   ...
								   num upto, INSRIP& dimsimps, num callerind)
{
	...
	// otherwise, iterate over nblist, get vertex v
	typename std::vector<num>::const_iterator niter;
	typename std::vector<num>::iterator intend;

	Simplex<C,PS,BT>* sigma;
	PT_SET siglist; // store vertices of tau in siglist
	std::vector<num> intersects, vnbrs;
	Point<PS>* v; // current vertex under consideration

	for (niter = nblist.begin(); niter != nblist.end(); ++niter)
	{
		intersects.clear();
		//std::cout<<" ind: "<<*niter; cin.get();
		siglist = tau->getVerts();
		v = vert.at(*niter); //extract vertex v from nblist
		if ((siglist.insert(v)).second) // augment vertices of tau with this
		{
			...
			{
				std::cout<<"\n***********intersect***************\n";
				c_print<std::vector<num> >(nblist); std::cout<<" and ";
				c_print<std::vector<num> >(vnbrs); std::cout<<" gives ";
				c_print<std::vector<num> >(intersects);
				std::cout<<"\n*************************************";
			}
			// and recursively call addcofaces!
...
template <typename C, typename PS, typename BT>
std::pair<num,bool> RIPS<C,PS,BT>::makeFromTimeSeriesFile(std::ifstream& infile)
{
	...
	{
		//cout<<"initopen\n";
		return std::make_pair(0,false); // error, couldn't open file for reading
	}
	...
	std::vector<PS> ptcoords;
	Point<PS>* curpt;

	std::set<Point<PS>*, ptcomplex<PS> > prmap;

	//cout<<"\n making pts frm ts"; cin.get();

	typename std::vector<PS>::const_iterator titer;
	...
	typename std::set<Point<PS>*, ptcomplex<PS> >::const_iterator pr;
	for(pr = prmap.begin(); pr != prmap.end(); ++pr)
	{
		vert.push_back(*pr);
		//cout<<"\n rad add: "<<pr->second;
