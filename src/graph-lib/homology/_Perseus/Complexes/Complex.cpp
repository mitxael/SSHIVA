...
template <typename C, typename BT>
std::pair<num,num> Complex<C,BT>::getSizeInfo(const BT& birth, const num& dim) const
{
	std::pair<num,num> toret = std::make_pair(0,0);
...
template <typename C, typename BT>
void Complex<C,BT>::printSizeInfo(bool dimdetail = false, bool framedetail = true, std::ostream& out = std::cout) const
{
	out<<"\n**********Complex Size: "<<size()<<"*************";
...
template <typename C, typename BT>
bool Complex<C,BT>::checkComplex(std::ostream& out = std::cout) const
{
	typename COMPLEX::const_iterator cit; // complex iterator
	typename FRAME::const_iterator fit; // frame iterator
...
template <typename C, typename BT>
void Complex<C,BT>::quicksert(const BT birth, const num dim, const std::vector<Cell<C,BT>*>& toins)
{
	...
	if (finder == clist.end()) // if not found...
	{
		...
		// create size info!
		curdsz = new DSIZE;
		(*curdsz)[dim] = std::make_pair(0,0); // initialize to nothing for this new birth + dimension...
	...
	else // if found,...
	{
		...
		if (dimloc == curf->end()) // not found!
		{
			...
			// size info!
			(*curdsz)[dim] = std::make_pair(0,0); // initialize to nothing for this new dimension...
		}
		else
		{
			curl = dimloc->second;
...
template <typename C, typename BT>
void Complex<C,BT>::printList(const BT& birth, const num dim, std::ostream& out = std::cout) const
{
	typename COMPLEX::const_iterator cit; // complex iterator
	typename FRAME::const_iterator fit; // frame iterator
	typename CLIST::const_iterator lit; // list iterator
...
template <typename C, typename BT>
void Complex<C,BT>::printFrame(const BT& birth, std::ostream& out = std::cout) const
{
	typename COMPLEX::const_iterator cit; // complex iterator
	typename FRAME::const_iterator fit; // frame iterator
	typename CLIST::const_iterator lit; // list iterator
...
bool Complex<C,BT>::insertCell(Cell<C,BT>* toin)
{
	...
	if (findbt == clist.end()) //if we don't have this frame,...
	{
		...
		myframe->insert(std::make_pair(dim,mylist));
		// and this frame into the complex
		clist.insert(std::make_pair(born,myframe));

		// also create size information!
		cursz = new DSIZE;
		(*cursz)[dim] = std::make_pair(1,0); // 1 cell of this dim, with index 0
...
template <typename C, typename BT>
std::pair<num,bool> Complex<C,BT>::getBotDim(const BT& born) const
{
...
template <typename C, typename BT>
std::pair<num,bool> Complex<C,BT>::getTopDim(const BT& born) const
{
...
template <typename C, typename BT>
std::pair<num,bool> Complex<C,BT>::getTopDim() const
{
	//cout<<"\ndetermining topdim... ";
	...
	return std::make_pair(toret,flag);
}

// gets bottom dimension of the whole complex!
template <typename C, typename BT>
std::pair<num,bool> Complex<C,BT>::getBotDim() const
{
	//cout<<"\ndetermining topdim... ";
	typename COMPLEX::const_iterator mit;
...
template <typename C, typename BT>
void Complex<C,BT>::printSizeStruct(std::ostream& out = std::cout) const
{
...
// helper function to output the complex:
template <typename C, typename BT>
void Complex<C,BT>::print (std::ostream& out = std::cout) const
{
	typename COMPLEX::const_iterator cit; // complex iterator
